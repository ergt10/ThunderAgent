...
# ============================================================================
# SkyRL + Harbor: CodeContests on 8x H100 with Docker sandboxes
#
# Usage:
#   ./run_codecontest_8xh100_docker.sh smoke   # Stage 1: verify the loop
#   ./run_codecontest_8xh100_docker.sh pilot   # Stage 2: real RL updates
#   ./run_codecontest_8xh100_docker.sh full    # Stage 3: full training
#
# Prerequisites:
...
# wandb api key (uncomment and set for pilot/full runs)
...
# Directory setup
#-----------------------
RUN_NAME="${RUN_NAME_OVERRIDE:-codecontest-8xh100-docker-${STAGE}}"
...
MONITORING_DIR="$RUN_ARTIFACT_DIR/monitoring"
...
MODEL_NAME="Qwen3-8B"
MAX_MODEL_LEN=32768
TRAIN_MAX_SEQ_LEN="$MAX_MODEL_LEN"
...
#-----------------------
# Placement: 4 GPUs training (policy+ref), 4 GPUs rollout
#-----------------------
TRAIN_GPUS=4
ROLLOUT_ENGINES=4

#-----------------------
# Training defaults (Dr. GRPO with KL loss to activate ref model)
#-----------------------
...
case "$STAGE" in
  smoke)
    ENTRYPOINT="examples.train_integrations.harbor.entrypoints.main_harbor_generate"
    MAX_TRAIN_TASKS=32
    MAX_EVAL_TASKS=10
    N_SAMPLES=1
    EVAL_N_SAMPLES=1
...
if [ "$AUTO_START_MONITOR" = true ]; then
  ...
  MONITOR_PID=$!
  echo "Started Stage 3 monitor pid=$MONITOR_PID output_dir=$MONITORING_DIR tensorboard_dir=$TENSORBOARD_DIR"
fi
